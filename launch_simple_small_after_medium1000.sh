...
MEDIUM_RUN=${MEDIUM_RUN:-fixed_blocks_simple64_manual400_compressed_answer_fa4pad025_q231_r530_a263_dwdseonly_seed10_r32a64_b16ga8_step5000}
...
MEDIUM_OUT=/root/autodl-tmp/sedd_outputs/lora_${MEDIUM_RUN}
MEDIUM_CKPT=${MEDIUM_OUT}/checkpoint_step_1000.pt
...
eval_ckpt() {
  ...
  if [ -f "${out_json}" ]; then
    ...
    log "starting_eval run=${run} ckpt=${ckpt} pretrained=${pretrained}"
...
run_small_experiment() {
  ...
  if [ -f "${ckpt}" ]; then
    ...
    log "launch_small_train run=${run} max_steps=${max_steps} num_t=${num_t} t_grid=${t_grid:-random}"
  ...
  eval_ckpt "${run}" "${ckpt}" "step${max_steps}" "pretrained/sedd-small"
}

log "small_after_medium1000_controller_start medium_run=${MEDIUM_RUN}"
wait_for_file "${MEDIUM_CKPT}" "medium_ckpt1000"
stop_run_if_alive "${MEDIUM_RUN}"
eval_ckpt "${MEDIUM_RUN}" "${MEDIUM_CKPT}" "step1000" "pretrained/sedd-medium"
...
log "small_after_medium1000_controller_done"
