...
### 13. 固定分区 compressed reasoning 训练方案

时间：2026-06-02

本轮切换为固定 token 区间训练，目标是减少模型学习输出格式的负担，把最终答案放进独立 answer block，评估时直接比较 answer block，不再从长 reasoning 文本中搜索 `\boxed{...}`。
...
question: [0,231)
reasoning: [231,761)
final_answer: [761,1024)
reasoning_block_len: 530
final_answer_block_len: 263
```

数据处理：

```text
source: data/compression_pilot/deepseek_q220_r512_parts_v3/merged_train.json
rows: 599
record fields: question, solution, source_id, metadata
final_answer: 从 solution 最后一个 \boxed{...} 中提取 boxed inner
reasoning: solution 去掉最后一个 \boxed{...}
missing_final_answer: 0
mask_overlap: 0
reasoning_len_min_max_avg: 93 501 296.3
final_answer_len_min_max_avg: 2 114 6.6
```

实现文件：

```text
data_sft.py: SFTFormatConfig.fixed_layout 与 format_fixed_sft_record
train_sft.py: fixed_layout 参数、answer_window_len batch 字段
eval_fixed_blocks.py: 固定分区采样与 answer-block 直接评估
run_fixed_blocks_sft.sh: 固定分区 smoke/64 样本训练入口
run_fixed_blocks_eval64.sh: 64 条训练样本评估入口
run_fixed_blocks_full599_sft.sh: 599 全样本 471/64/64 训练入口
run_fixed_blocks_eval_splits.sh: valid/test split 评估入口
watch_fixed_blocks_eval64.sh: 64/10000 完成后自动 eval64 + 汇总
watch_fixed_blocks_eval_splits.sh: full599/100000 完成后自动 valid/test eval + 汇总
launch_fixed_blocks_full599.sh: full599 训练与 valid/test watcher 的统一 nohup 启动入口
status_fixed_blocks_run.sh: 只读状态查看入口，统一输出进程、step、checkpoint、watcher、eval summary；train64 默认查 train64 JSONL，full599 默认查 valid/test JSONL
tools/summarize_fixed_eval.py: JSONL 评估结果汇总
tools/check_fixed_eval_gate.py: eval64 后检查 answer/eos/empty gate，给出是否启动 full599 的建议
tools/analyze_fixed_eval_failures.py: eval JSONL 失败模式分析，统计 no_eos/empty/long_pred/repetition 等退化类型
launch_full599_if_eval64_pass.sh: 可选入口；eval64 gate PASS 后启动 full599，支持 DRY_RUN=1
watch_full599_after_eval64_gate.sh: 等待 eval64 完整产出，gate PASS 后自动调用 full599 launcher；gate FAIL 则停止并记录失败原因
```

Eval 诊断字段：

```text
eval_fixed_blocks.py JSONL 额外记录 eos_index、pred_answer_token_len、answer_block_token_len、raw_answer_block_prefix、answer_token_ids_prefix。
tools/summarize_fixed_eval.py 汇总 eos_index 与 pred_answer_token_len，失败样本打印 eos_idx/pred_tokens，便于区分 answer 错误、EOS 缺失和块内重复退化。
tools/summarize_fixed_eval.py 默认额外打印失败样本 raw_answer_block_prefix，受 --raw-width 控制。
tools/analyze_fixed_eval_failures.py 进一步统计 failure_mode_combos；status_fixed_blocks_run.sh 在发现 eval JSONL 后会自动调用 summarizer、failure analyzer 和 gate checker。
watch_fixed_blocks_eval64.sh 在 eval64 完成后会自动调用 summarizer、failure analyzer 和 gate checker；watch_fixed_blocks_eval_splits.sh 在 valid/test eval 完成后会自动调用 summarizer 和 failure analyzer。
```

训练/推理固定分区一致性审计：

```text
时间: 2026-06-02 20:05 CST
状态: 通过
样本: train64 split 的 idx=0,1,2,31,63
检查项:
- data_sft.py 训练格式固定 question=[0,231), reasoning=[231,761), final_answer=[761,1024)
- train_sft.py DWDSE 使用 prompt_len=231 与 answer_window_len=793，只在 answer_mask & pad_mask 上计 loss
- eval_fixed_blocks.py clamp question=[0,231)，采样 generation=[231,1024)，直接比较 final_answer=[761,1024)
- 抽样样本的 prompt_mask block、answer_mask region、pad_mask real-token 计数和 final answer 解码全部一致
结论: 训练和推理评估的区间/字段/答案块没有发现错位；后续 eval64 若失败，应优先看生成退化/过拟合不足，而不是 boxed 搜索或区间错配。
```

Smoke 训练：

```text
run_name: fixed_blocks_smoke64_q231_r530_a263_dwdseonly_seed10_r32a64_b8ga4_step200
train_size: 64
max_steps: 200
batch_size: 8
grad_accum: 4
loss: response-only DWDSE
all_mask_ce_weight: 0
fixed_layout_supervise_pad: false
result: checkpoint 与 lora_final.pt 正常保存
loss: 4.4899 -> 3.2541
```
...
```text
ckpt: /root/autodl-tmp/sedd_outputs/lora_fixed_blocks_smoke64_q231_r530_a263_dwdseonly_seed10_r32a64_b8ga4_step200/lora_final.pt
eval: train16, argmax64
answer_block_match: 0/16 = 0.0%
exact_answer_match: 0/16 = 0.0%
no_eos_answer_blocks: 16/16 = 100.0%
结论: 固定分区训练/推理链路可跑通，但 200 step 太浅，answer block 仍是重复符号/片段。
```

当前 64 样本 10000 step 训练：

```text
run_name: fixed_blocks_train64_q231_r530_a263_dwdseonly_seed10_r32a64_b16ga8_step10000
out_dir: /root/autodl-tmp/sedd_outputs/lora_fixed_blocks_train64_q231_r530_a263_dwdseonly_seed10_r32a64_b16ga8_step10000
log: /root/sedd/logs/fixed_blocks/nohup_fixed_blocks_train64_q231_r530_a263_dwdseonly_seed10_r32a64_b16ga8_step10000.log
train_size: 64
max_steps: 10000
batch_size: 16
grad_accum: 8
effective_update_samples: 128
lr: 3e-4
warmup_steps: 50
lora_r: 32
lora_alpha: 64
...
```text
step=50  loss=3.7316
step=100 loss=3.1691
step=150 loss=2.9508
step=200 loss=2.7862
step=250 loss=2.5605
step=300 loss=2.4991
step=350 loss=2.2624
step=400 loss=2.1978
step=450 loss=2.0828
step=500 loss=2.0237
step=550 loss=1.9128
step=600 loss=1.8390
step=650 loss=1.7312
step=700 loss=1.7107
step=750 loss=1.6322
step=800 loss=1.4766
step=850 loss=1.4401
step=900 loss=1.4185
step=950 loss=1.3502
step=1000 loss=1.3042
step=1050 loss=1.2591
step=1100 loss=1.2054
step=1150 loss=1.1698
step=1200 loss=1.1380
step=1250 loss=1.0485
step=1300 loss=1.0423
step=1350 loss=1.0329
step=1400 loss=0.9995
step=1450 loss=0.9596
step=1500 loss=0.9554
step=1550 loss=0.9414
step=1600 loss=0.9095
step=1650 loss=0.8862
step=1700 loss=0.8694
step=1750 loss=0.8670
step=1800 loss=0.8640
step=1850 loss=0.8298
step=1900 loss=0.8073
step=1950 loss=0.8296
step=2000 loss=0.8204
step=2050 loss=0.8092
step=2100 loss=0.7906
step=2150 loss=0.7768
step=2200 loss=0.7679
step=2250 loss=0.7518
step=2300 loss=0.7514
step=2350 loss=0.7516
step=2400 loss=0.7781
step=2450 loss=0.7427
step=2500 loss=0.7546
step=2550 loss=0.7348
step=2600 loss=0.7469
step=2650 loss=0.7287
step=2700 loss=0.7159
step=2750 loss=0.7290
step=2800 loss=0.7281
step=2850 loss=0.6878
step=2900 loss=0.7103
step=2950 loss=0.7027
step=3000 loss=0.7005
mem: ~23.9GB / 32GB
status: 2026-06-02 21:15 CST 训练进程仍在运行，GPU 利用率约 99%，已生成 step1000/step2000/step3000 checkpoint，暂无 eval64；loss 已降到 0.7005，继续等待 step10000 final 和 train64 eval
checkpoint_step_1000.pt: /root/autodl-tmp/sedd_outputs/lora_fixed_blocks_train64_q231_r530_a263_dwdseonly_seed10_r32a64_b16ga8_step10000/checkpoint_step_1000.pt
checkpoint_step_2000.pt: /root/autodl-tmp/sedd_outputs/lora_fixed_blocks_train64_q231_r530_a263_dwdseonly_seed10_r32a64_b16ga8_step10000/checkpoint_step_2000.pt
checkpoint_step_3000.pt: /root/autodl-tmp/sedd_outputs/lora_fixed_blocks_train64_q231_r530_a263_dwdseonly_seed10_r32a64_b16ga8_step10000/checkpoint_step_3000.pt
checkpoint_last.pt: /root/autodl-tmp/sedd_outputs/lora_fixed_blocks_train64_q231_r530_a263_dwdseonly_seed10_r32a64_b16ga8_step10000/checkpoint_last.pt
```

运行参数审计：

```text
时间: 2026-06-02 20:08 CST
状态: 通过
source: /root/autodl-tmp/sedd_outputs/lora_fixed_blocks_train64_q231_r530_a263_dwdseonly_seed10_r32a64_b16ga8_step10000/args.json 与 python pid=27925 cmdline
fixed_layout: true
blocks: 231 + 530 + 263 = 1024
train/valid/test: 64/0/0
loss: response-only DWDSE, all_mask_ce_weight=0.0
batch_size: 16
grad_accum: 8
effective_update_samples: 128
lora: r=32 alpha=64 dropout=0.0 targets=attn_qkv,attn_out,mlp.0,mlp.2
max_steps: 10000
save/log/eval: save_freq=1000 log_freq=50 eval_freq=0
```

自动评估：

```text
watcher: watch_fixed_blocks_eval64.sh
watcher_pid: 35701
watch_log: /root/sedd/logs/fixed_blocks/watch_eval_fixed_blocks_train64_q231_r530_a263_dwdseonly_seed10_r32a64_b16ga8_step10000.log
behavior: 训练进程结束并检测到 lora_final.pt 后，自动运行 run_fixed_blocks_eval64.sh，并调用 tools/summarize_fixed_eval.py 汇总 match/exact/eos/empty/失败样例。
fix: 2026-06-02 19:05 发现旧 watcher 进程 PATH 缺少 /root/miniconda3/bin；已给 watch_fixed_blocks_eval64.sh 增加 PATH，并重启 watcher(pid=30804)，训练 pid=27920 未变。
fix: 2026-06-02 19:28 为确保运行中的 watcher 使用最终版脚本，重启 watcher(pid=32003)，训练 pid=27920 未变。
fix: 2026-06-02 20:15 为让运行中的 watcher 使用新增 failure analyzer 与 gate checker，重启 eval watcher(pid=35701)，训练 pid=27920/python pid=27925 未变。
gate watcher: watch_full599_after_eval64_gate.sh 低频等待 eval64 完整 JSONL；若 match>=90%、eos>=90%、empty<=5% 则启动 full599，否则不启动。2026-06-02 19:54 已启动 pid=33556。
```

放大到 full599 前检查：

```text
必须先读取 64/10000 最终 eval64：
- answer_block_match / exact_answer_match
- eos_present / empty_answer_blocks
- failure samples 的 pred_answer 与 generated_reasoning
- tools/check_fixed_eval_gate.py 默认 gate: match>=90%, eos>=90%, empty<=5%

若 64 样本仍无法过拟合或 answer block 大量无 EOS/重复退化，先排查固定分区监督或采样设置，不直接启动 599/100000。
若 64 样本 answer block 已基本可拟合，可运行 launch_full599_if_eval64_pass.sh 或 launch_fixed_blocks_full599.sh；当前也准备了 watch_full599_after_eval64_gate.sh，可在 eval64 gate PASS 后自动衔接 full599。
```

全样本计划：

```text
script: run_fixed_blocks_full599_sft.sh
launcher: launch_fixed_blocks_full599.sh
data_json: data/compression_pilot/deepseek_q220_r512_parts_v3/merged_train.json
split: train=471 valid=64 test=64
split_overlap: 0
split_audit_2026-06-02_20:03_CST: train rows=471 unique=471 missing_final_answer=0 reasoning_len=93..501 answer_len=2..79; valid rows=64 unique=64 missing_final_answer=0 reasoning_len=104..482 answer_len=2..55; test rows=64 unique=64 missing_final_answer=0 reasoning_len=119..497 answer_len=2..114; overlap train/valid=0 train/test=0 valid/test=0; total=599
disk_audit_2026-06-02_20:10_CST: /root/autodl-tmp total=50G used=30G avail=21G use=59%; /root/autodl-tmp/sedd_outputs=2.4G, checkpoint_files=33, total_pt_gb=2.33；full599 LoRA checkpoint 预计不会触发磁盘容量风险
max_steps: 100000
batch_size: 16
grad_accum: 8
save_freq: 5000
eval after train: run_fixed_blocks_eval_splits.sh, default valid/test
watcher after train: watch_fixed_blocks_eval_splits.sh
launcher guard: 默认若发现其他 train_sft.py 正在运行则退出；需要并发时显式 ALLOW_CONCURRENT_TRAIN=1
process check: launcher/watcher 使用 [t]rain_sft.py pgrep pattern，避免检查命令自匹配。
static checks: local bash -n/py_compile ok; remote bash -n/py_compile ok
sync checks: 2026-06-02 20:01 CST，eval/gate/full599 相关 12 个关键脚本本地与远端 sha256 一致；本地 bash -n/py_compile ok，远端 bash -n 与 /root/miniconda3/bin/python py_compile ok
```
